...
class ConversationRepositoryImpl(
    ...
    override fun saveAggregate(conversation: Conversation, userId: String?) {
        ...
        // 会話全体で使われるGrammar
        val grammarEntities = conversation.grammars.map { grammarRef ->
            ConversationGrammarEntity(
                id = UUID.randomUUID().toString(),
                conversationId = conversation.id,
                grammarId = grammarRef.id,
                createdAt = conversation.createdAt
            )
        }
        conversationGrammarEntityRepository.saveAll(grammarEntities)
